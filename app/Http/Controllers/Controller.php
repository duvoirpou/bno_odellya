...
class Controller extends BaseController
{
    ...
    {
        $ouvrages = Ouvrage::orderBy('created_at', 'desc')
            ->paginate(6);
    ...
    {
        $ouvrage = Ouvrage::select('ouvrages.*')
        ->where('slug', $slug)
        ->first();

        $count_ouvrage = Ouvrages_vu::select(Ouvrages_vu::raw('COUNT(ouvrages_vus.ouvrages_id) AS total'))
        ->where('ouvrages_vus.ouvrages_id', $ouvrage->id)
        ->first();

        //recupere l'ip de l'utilisateur
        $ip = $_SERVER['REMOTE_ADDR'];

        //ne pas repeter à la fois la meme addresse ip et le meme id de la propriété dans la table proprietes_vues
        if (Ouvrages_vu::where('users_ip', $ip)->where('ouvrages_id', $ouvrage->id)->doesntExist()) {
            $vue = new Ouvrages_vu;
            $vue->users_ip = $ip;
            $vue->ouvrages_id = $ouvrage->id;
            $vue->save();
        }

        return view('show-ouvrage', compact('ouvrage', 'count_ouvrage'));
    }
